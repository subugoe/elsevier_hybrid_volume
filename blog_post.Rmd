...
description: Publishers rarely make invoice data for hybrid open access publication fees transparent. Elsevier is an remarkable exception, because the publisher provides machine-readable information about open access agreements and fee waivers at the article level. This blogpost demonstrates how to mine these data from Elsevier full-texts with R. Analysing the resulting dataset of 70,000 hybrid open access articles published in around 1,800 journals between 2015 and now reveals a growth of publication fees paid through agreements with funders and libraries. Nevertheless, the majority of funding sources for hybrid open access articles remains unknown, raising important questions about the transparency of fee-based open access publishing.
date: Aug 29, 2019
author:
  - name: Najko Jahn 
...
As a start, I used the [Elsevier publication fee price list](https://www.elsevier.com/__data/promis_misc/j.custom97.pdf), shared as pdf document, to determine hybrid open access journals in Elsevier's journal portfolio. The rOpenSci [tabulizer](https://github.com/ropensci/tabulizer) package allowed to extract data about these journals from this file. 
...
After downloading the Elsevier full-texts with the [crminer](https://docs.ropensci.org/crminer/) package, a client maintained by rOpenSci, I extracted the above-highlighted open access informatiom from the `xml` documents. 

<aside>
  See [R/tdm_oa_info.R](https://github.com/njahn82/elsevier_hybrid_volume/blob/master/R/tdm_oa_info.R)
</aside>

Moreover, I parsed the first author email address, assuming that email domains roughly indicate the affiliation of the first respective corresponding author at the time of publication. The package [urltools](https://CRAN.R-project.org/package=urltools) enabled to extract email domains and to split them in meaningful parts. 
...
In the following data analysis, I will be using two files. The first file, [`journal_facets.json`](journal_facets.json), contains the number of publications per Elsevier hybrid open access journal and year. It furthermore provides the various license URLs found through Crossref.

The second file comprises article-level data. Each row holds information for a single hybrid open access article, and the columns represent:
...
```{r, echo = FALSE, fig.caption = 'Hybrid open access uptake in Elsevier journals per year in percent. '}
library(tidyverse)
library(jsonlite)
library(ggeconodist)
...
In most cases, payment notifications were send to the authors (`r round(hybrid_df %>% filter(oa_sponsor_type == "Author") %>% nrow() / nrow(hybrid_df) * 100, 2)` %). For around one third of articles, Elsevier lists a funding body covering the open access publication fee.
...
Mostly British and Dutch funders sponsored hybrid open access in Elsevier journals. But also the German Federal Ministry of Education and Research (BMBF) is well represented despite the [current boycott from most universities and research organizations in Germany](https://www.nature.com/articles/d41586-018-05754-1). Since 2018, the BMBF has financially supported `r hybrid_df %>% filter(grepl("BMBF", oa_sponsor_name)) %>% nrow()` hybrid open access articles that appeared in `r hybrid_df %>% filter(grepl("BMBF", oa_sponsor_name)) %>% distinct(issn) %>% nrow()` Elsevier journals according to the publisher.
