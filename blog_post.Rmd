...
To invite discussion about Elsevier's approach and its potential for monitoring transformative agreements, I made the resulting dataset openly available on GitHub along with the source code used to obtain the data.
...
Finally, to measure the overlap between crowd-sourced and publisher-provided invoice data, I downloaded spending data from the Open APC Initiative [@openapc], which is, to my knowledge, the largest evidence-base for institutional spending on open access publication fees.
...
The following interactive visualization (Figure \@ref(fig:treefunder)), created with the [echarts4r](https://echarts4r.john-coene.com/) package[@echarts], let you browse the invoicing data.
...
In the following, a hierarchical, interactive treemap visualises the distribution of the email domains (see Figure \@ref(fig:emailsub)). It appears that it roughly represents the national research landscapes. However, the dominance of domains from commercial organizations, mostly email providers like "gmail.com" or the Chinese "163.com" and "126.com" highlights the limitations of this approach to infer eligble funding institutions from author email adresses.
...
The last figure presents an gap analysis between publisher-provided invoice data and Open APC for the ten most important funding bodies  measured by the number of hybrid open access articles. Figure \@ref(fig:overlap) highlights that British funders had the largest overlap rates, reflecting Open APC efforts to re-use openly available spending data from these institutions [@Pieper_2018]. On the other hand, Open APC did not track Dutch ("VSNU"), US-American ("Melinda & Bill Gates Foundation") and European funding activities ("European Research Council") for hybrid open access publication fees. 
