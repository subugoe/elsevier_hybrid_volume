...
As a start, I used the [Elsevier publication fee price list](https://www.elsevier.com/__data/promis_misc/j.custom97.pdf), an openly available pdf document, to determine the current hybrid open access journals in Elsevier's journal portfolio. The rOpenSci [tabulizer](https://github.com/ropensci/tabulizer) package [@tabulizer] allowed to extract data about these journals from this file. 
...
In the following, a hierarchical, interactive treemap visualizes the distribution of the email domains (see Figure \@ref(fig:emailsub)). It appears that the distribution of email domains roughly represent the national research landscapes. However, the dominance of domains from commercial organizations, mostly email providers like "gmail.com" or the Chinese "163.com" and "126.com" highlights the limitations of this approach to infer eligble funding institutions from author email adresses.
...
Finally, I was interested in the overlap between publisher-provided invoice data from Elsevier and institunal spending for hybrid open access publications disclosed by the Open APC initiative. In total, `r hybrid_tmp %>% filter(in_openapc == TRUE) %>% nrow()` out of `r nrow(hybrid_tmp)` hybrid open access articles were also tracked by the Open APC initiative, corresponding to an share of `r round(hybrid_tmp %>% filter(in_openapc == TRUE) %>% nrow() /  nrow(hybrid_tmp) * 100, 2)` %. However, the Open APC initiative listed another `r nrow(elsevier_miss)` hybrid open access articles, presumably because they did not meet my metadata requirements, or appeared in journals that transitioned from hybrid to fully open access recently (e.g. the journal NeuroImage). At the journal level, the overlap was `r round(hybrid_tmp %>% filter(in_openapc == TRUE) %>% distinct(issn) %>% nrow() / length(unique(hybrid_tmp$issn)) * 100, 2)` %.
...
hybrid_tmp %>%
  ...
  scale_fill_manual("Elsevier Hybrid Open Access Articles",
                    values = c("#b3b3b3a0", "#56B4E9"), 
                    labels = c("All", "in Open APC")) +
  labs(x = NULL, y = NULL) + 
       #title = "Elsevier Hybrid Open Access:\nTo whom did the publisher send the bill?",
      # caption = "Data source: Elsevier B.V. / Crossref") +
  scale_y_continuous(labels = scales::number_format(big.mark = " ")) +
...
hybrid_tmp %>% 
 # filter(oa_sponsor_type %in% c("Author", "FundingBody")) %>%
  ...
  geom_bar(stat = "identity") +
  scale_fill_manual("", 
                    values = c("#b3b3b3a0", "#56B4E9"), labels = c("All", "in Open APC")) +
  scale_y_continuous(labels = scales::number_format(big.mark = " ")) +
  coord_flip() +
  labs(x = NULL, y = NULL) +
  theme_minimal(base_family="Roboto", base_size = 12) +
