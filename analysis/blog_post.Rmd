...
<aside>
  See [R/fetch_apc_list.R](https://github.com/njahn82/elsevier_hybrid_volume/blob/master/R/fetch_apc_list.R) to understand how to obtain journal data.
</aside>
...
<aside>
  See [R/fetch_cr_md.R](https://github.com/njahn82/elsevier_hybrid_volume/blob/master/R/fetch_cr_md.R) to understand how Crossref was called with R.
</aside>
...
<aside>
  See [R/tdm_oa_info.R](https://github.com/njahn82/elsevier_hybrid_volume/blob/master/R/tdm_oa_info.R) to understand how open access information was extracted from `xml` documents.
</aside>
...
<aside>
  See [R/extract_email_domains.R](https://github.com/njahn82/elsevier_hybrid_volume/blob/master/R/extract_email_domains.R) to understand how email addresses were parsed.
</aside>
...
Throughout the data analysis, I used tools from the Tidyverse [@Wickham_2019].
...
a_plot <- perc_df %>%
  ...
  #  subtitle = "Proportion of immediate open access articles published in subscription-based journals\nJan 2015 to July 2019",
   # caption = "Data source: Crossref, Elsevier B.V."
  ) +
...
```{r treefunder, fig.cap="Breakdown of Elsevier hybrid open access journal articles by invoice recipient. Each rectangle represents an invoicing type and can be broken down by recipient. Data Source: Crossref, Elsevier B.V."}
library(echarts4r)
hybrid_df %>%
  mutate(oa_sponsor_type = recode(oa_sponsor_type,
                                  `FundingBody` = "Agreement",
...
```{r emailsub, fig.cap="Email domain analysis of first resp. corresponding authors publishing open access in Elsevier hybrid journals. Each top-level domain can be subdivided further into domain names representing academic institutions or companies. Data Source: Crossref, Elsevier B.V."}
library(echarts4r)
hybrid_df %>%
...
Implementation of Plan S is underway to change current practises of funding  open access publication in hybrid journals. Because Elsevier's <!--delete:current--> transparency related to their invoicing practices is an remarkable exception, workflow guidelines for transformative agreements should consider taking in the publisher's example of sharing invoice data as a recommended good business practice for publishers. Although future work needs to tackle the remaining questions about the data quality and coverage, publisher-provided invoice data makes publishers more accountable and extends the evidence base relative to <!--related to?--> hybrid open access. As a result, the data analysis presented here provides a basis to improve the monitoring of funding streams in the context of transformative agreements.
